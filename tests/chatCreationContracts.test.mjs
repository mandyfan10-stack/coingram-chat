...
const chatActions = readFileSync(new URL('../src/context/chat/useChatActions.js', import.meta.url), 'utf8');
...
test('group and channel creation is atomic', () => {
  assert.match(chatService, /\.rpc\('create_managed_chat'/);
  assert.match(chatService, /p_member_ids:\s*memberIds/);
  assert.doesNotMatch(chatService, /const memberRows = \[\{ chat_id: newChat\.id/);
});
...
test('channel creation cannot leave the modal in an endless loading state', () => {
  assert.match(chatService, /const controller = new AbortController\(\)/);
  assert.match(chatService, /setTimeout\(\(\) => controller\.abort\(\), 15000\)/);
  assert.match(chatService, /\.rpc\('create_managed_chat'[\s\S]*?\.abortSignal\(controller\.signal\)/);
...
test('successful managed chat creation does not wait for a full chat-list refresh', () => {
  ...
  assert.doesNotMatch(chatActions, /else \{\s*await fetchChats\(\);/);
});
